...
# echo "Đường dẫn của tệp lịch sử là: $HISTORY_FILE"
#   while IFS=: read -r value1 value2; do
#       echo "Value 1: $value1"
#       echo "Value 2: $value2"
#       ./deloychaincode.sh $value1 $value2
...
function networkDown() {
     WALTE_DIR="../sever-managent/wallet"  # Đảm bảo rằng WALTE_DIR trỏ đúng tới thư mục cần xóa

    # Kiểm tra xem thư mục wallet có tồn tại không
    if [ -d "$WALTE_DIR" ]; then
        echo "Đang xóa dữ liệu trong thư mục walte..."

        # Xóa tất cả các tệp trong thư mục WALTE_DIR nhưng không xóa thư mục chính
        rm -rf "$WALTE_DIR"/*  # Lưu ý: chỉ xóa các tệp trong thư mục, không xóa thư mục chính

        echo "Dữ liệu trong thư mục walte đã bị xóa."
    else
        echo "Thư mục walte không tồn tại. Không cần xóa dữ liệu."
    fi

    echo "Đã đóng network và xóa dữ liệu thành công."
